...
class LeapTrackingThread(threading.Thread):
    ...
    @staticmethod
    def reconstruct_hands_from_flattened(flat_list: list[float]) -> list[dict]:
        """
        Convert a flattened list generated by get_latest_hands_flattened()
        back into the structured list[dict] format.
        """
        if not flat_list or len(flat_list) < 1:
            return []

        reconstructed_hands = []
        num_hands = int(flat_list[0])
        
        # Stride per hand: 1 float (type) + (21 landmarks * 3 coordinates) = 64 floats
        hand_stride = 64
        idx = 1

        for _ in range(num_hands):
            if idx + hand_stride > len(flat_list):
                break  # Protects against malformed or truncated data

            # Extract hand type (0.0 -> left, 1.0 -> right)
            hand_type = "left" if flat_list[idx] == 0.0 else "right"

            # Parse the 63 floats (21 coordinate pairs)
            kp_start = idx + 1
            kp_end = kp_start + 63
            flat_kps = flat_list[kp_start:kp_end]

            # Reconstruct into list of (x, y, z) tuples
            keypoints_list = []
            for i in range(21):
                x = flat_kps[i * 3]
                y = flat_kps[i * 3 + 1]
            ...
            # Build dict using original landmark names
            keypoints_dict = {
                name: keypoints_list[i] 
                for i, name in enumerate(LANDMARK_NAMES)
            }

            # Reconstruct the original structure
            # Note: Hand ID and Palm Position were not stored in the flat list, 
            # so they default to None and (0.0, 0.0, 0.0) respectively.
            reconstructed_hands.append({
                "id": None,
                "type": hand_type,
